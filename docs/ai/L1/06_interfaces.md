...
> Boundary contracts: backend routes, the `/api/*` rewrite map, env vars, the response envelope, the `CustomLLM` vendor config, and the OpenAI SSE contract.

## Backend routes (port 8000)

The browser calls the first three as `/api/<name>`; Next rewrites to `/<name>`. Agora cloud calls `/llm/chat/completions` directly.
...
- 400 if `AGORA_APP_ID`/`AGORA_APP_CERTIFICATE`/`CUSTOM_LLM_URL`/`CUSTOM_LLM_API_KEY` are unset, or `channelName`/`rtcUid`/`userUid` invalid.
...
- Caller: Agora ConvoAI Cloud (not the browser).
- Body: OpenAI `ChatCompletionRequest` (`stream: true` required; `stream: false` → 400).
- Returns: OpenAI SSE stream ending with `data: [DONE]`. See [filter_seam](L2/filter_seam.md) for chunk structure.
- `Authorization: Bearer <CUSTOM_LLM_API_KEY>` is forwarded by Agora cloud; the mock does not validate it.

### `GET /llm/health`

- Returns `{ "status": "ok", "service": "content-filter-llm" }`.
...
`data` omitted when the route has no payload. Non-zero `code` or missing `data` = error on the client side. **Does not apply to `/llm/*` routes** (those use OpenAI SSE format).
...
| `CUSTOM_LLM_URL`        | backend            |    ✅    | — (must be public URL) |
| `CUSTOM_LLM_API_KEY`    | backend            |    ✅    | `any-key-here` |
| `CUSTOM_LLM_MODEL`      | backend            |          | `filter-mock` |
| `AGENT_GREETING`        | backend            |          | built-in line |
| `FILTER_BANNED_TERMS`   | backend (llm.py)   |          | `strawberries` |
...
- `base_url` = `CUSTOM_LLM_URL` (full URL including `/llm/chat/completions`).
- `api_key` = `CUSTOM_LLM_API_KEY` (forwarded as `Authorization: Bearer` by Agora cloud).
- STT: `DeepgramSTT(model="nova-3", language="en")`.
- TTS: `MiniMaxTTS(model="speech_2_6_turbo", voice_id="English_captivating_female1")`.
- `turn_detection` is configured on `AgoraAgent(...)` directly (not on the vendor) with `mode: "vad"` for both start-of-speech and end-of-speech — see [07_gotchas](07_gotchas.md).

## Related Deep Dives

- [filter_seam](L2/filter_seam.md) — OpenAI SSE chunk structure, `moderate()` signature, and swap patterns.
